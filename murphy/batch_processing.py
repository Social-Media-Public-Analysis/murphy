...
class Batches:
    ...
    def process_in_batches(file_paths: Iterable[str], read_func: Callable[[str], Any],
        ...
        """
        Function to process data in batches to circumvent Dask Scheduler's limitations (max of 100k
        tasks for example)
        :param file_paths: path of files that need to be individually processed
        :param read_func: function to read the file. This must return an object (for example: Dask
        Bag, Dask Array, str)
    ...
    def process_in_batches_generator(file_iterator: Iterable[str], read_func: Callable[[str], Any],
        ...
        """
        Function to process data in batches to circumvent Dask Scheduler's limitations for 100k
        tasks
